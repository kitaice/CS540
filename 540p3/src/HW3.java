...
public class HW3 {
	...
	public static void main(String[] args) {
		...
		if (mode == 0) {
			//TODO: add code here.
    		DecisionTreeImpl tree = new DecisionTreeImpl(mTrainDataSet,mTrainAttributeNames);
            tree.printInfo();
		}else if(mode == 1){
			mTree.print();
		}else if(mode == 2){
            ...
            for(ArrayList<Double> instance : mTrainDataSet){
                classify=mTree.classify(instance);
                System.out.println(classify);
                if(classify==instance.get(mTrainAttributeNames.size())) accuracy++;
            }
            mTree.printAccuracy(accuracy,mTrainDataSet.size());
		}else if(mode == 3){
			//TODO: add code here.
            int classify;
            int accuracy=0;
            for(ArrayList<Double> instance : mTestDataSet){
                classify=mTree.classify(instance);
                System.out.println(classify);
                if(classify==instance.get(mTestAttributeNames.size())) accuracy++;
            }
            mTree.printAccuracy(accuracy,mTestDataSet.size());
		}else{

...
public class HW5 {
	...
	private static double[] evaluate(Label[] yTrue, Label[] yPred) {
		...
		for(int i=0;i<yTrue.length;i++){
			...
			if(yTrue[i]==Label.Positive && yPred[i]==Label.Positive) tp++;
			else if(yTrue[i]==Label.Negative && yPred[i]==Label.Positive) fp++;
			else if(yTrue[i]==Label.Negative && yPred[i]==Label.Negative) tn++;
			else fn++;
			//count the numbers of true positive, true negative, false positive, false negative
		}
		scores[0]=round3((double)(tp+tn)/(double)(tp+fp+tn+fn));
		scores[1]=round3((double)(tp)/(double)(tp+fp));
		scores[2]=round3((double)(tp)/(double)(tp+fn));
